...
- `base44 site dev` runs the site's `serveCommand` with no local backend, the frontend reaching its backend same-origin — the shape a hosted sandbox needs, where `base44 dev` (local backend) and `dev --remote` (the published app) are the developer-machine paths. It takes no arguments. Serving is its whole job, so it falls back to `npm run dev` when the block names none, and it binds the address from the config, forwarded to an `npm run`, `pnpm`, `yarn` or `bun run` script; a command that cannot take it is refused rather than served on a different address.
