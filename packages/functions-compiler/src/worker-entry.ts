...
function denoShimSource(): string {
  return (_denoShimSource ??= readFileSync(
...
function activationShimSource(): string {
  return (_activationShimSource ??= readFileSync(
...
export async function prepareFunction(
  ...
  return {
    ...
    files: {
      ...userFiles(files),
      ...workerRuntimeFiles(),
      ...(runtimeSecrets ? { [ACTIVATION_FILENAME]: activationShimSource() } : {}),
      [ENTRY_FILENAME]: buildEntrySource(entry, postResponseTelemetry, runtimeSecrets),
    },
  };
}
...
export function prepareApp(
  ...
  files[ENTRY_FILENAME] = buildAppEntrySource(moduleFiles, postResponseTelemetry, runtimeSecrets);
...
function buildEntrySource(userEntry: string, telemetry: boolean, runtimeSecrets = false): string {
  ...
  const returnExpr = runtimeSecrets ? `withoutActivationSignal(${handlerExpr})` : handlerExpr;
...
export function assertNoReservedFilenames(
  files: Record<string, string>,
): void {
