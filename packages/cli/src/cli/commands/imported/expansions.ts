...
const PROMPT_EXPANSIONS: Record<string, PromptExpansion> = {
  headless: {
    ...
    text: [
      ...
      "",
      "Verifying pages with the preview tools (preview_execute_code / preview_screenshot) — one adjustment for this app type: a Wix Headless app is multi-page (astro), not a single-page React app, so the default in-iframe navigation helper waits for a client-side route re-render that never happens here and times out as 'iframe did not respond', burning ~35s a call. So when you need to MOVE to another page, pass the reload option — window.__base44_preview.navigate(path, { reload: true }) — which does a real full page load and returns right away. For the same reason do NOT click an element that navigates to a new page (the full reload tears the page down mid-command); instead navigate to the target URL directly with { reload: true }, then verify. Read-only checks on the current page — window.__base44_preview.domSnapshot(), .inspect(selector), .consoleLogs({ level: 'error' }) — and preview_screenshot work normally and need no reload option; prefer them plus a screenshot for verifying appearance.",
    ].join("\n"),
  },
};
