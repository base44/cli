...
/** A finished tool replaces its own running line; everything else appends. */
function withEntry(
...
function statusText(status: SessionStatus, musingSeed: number): string {
  ...
  switch (status.phase) {
    ...
    case "running": {
      ...
      // Running tools show in place in the transcript (pulsing dot); this
      // line keeps the turn's own clock.
      ...
      const quiet =
        status.quietForMs > 60_000
          ...
          : "";
      return `${chalk.magenta(shimmer())} ${chalk.dim(`${idleMusing(musingSeed)} (${turnFor})${quiet}`)}`;
    }
    case "sending":
...
interface ViewProps {
  ...
  footer: string[];
  subscribe: (listener: (entry: TranscriptEntry) => void) => () => void;
}

function SessionView({ engine, footer, subscribe }: ViewProps) {
  const { exit } = useApp();
  const [items, setItems] = useState<TranscriptEntry[]>([]);
  const [verbose, setVerbose] = useState(false); // Ctrl+O: unfold every tool result
  ...
  const [card, setCard] = useState<CardState | null>(null); // the agent's question, when it has one
  const dismissedRef = useRef(new Set<string>()); // cards closed with Esc, until Tab
  const maxScrollRef = useRef(0);
  const meIdRef = useRef<string | null>(null);

  // Append a line straight into the transcript (for /command output that isn't
  // an engine event).
  const emit = (entry: TranscriptEntry) => setItems((h) => withEntry(h, entry));

  useEffect(
    // The trailing newline gives every stream item a blank line after it.
    () => subscribe((entry) => setItems((h) => withEntry(h, entry))),
    [subscribe],
  );
  useEffect(() => {
  ...
  // when the backend no longer reports that call waiting.
  ...
  useInput((char, key) => {
    ...
    // Tab reopens a card dismissed with Esc.
    if (key.tab && !card) {
      const next = pending.find((p) => dismissedRef.current.has(p.toolCallId));
      if (next) {
    ...
    // A card owns the keyboard — except while it is capturing text, when the
    // input box does and only Esc backs out.
    if (card) {
      const mapped: CardKey | null = key.upArrow
        ? "up"
        : key.downArrow
          ? "down"
          : key.return
      ...
      if (mapped) applyCard(cardKey(card, mapped));
      if (mapped || !card.typing) return;
    }
    if (key.ctrl && char === "c") {
      if (input) setInput("");
      else exit();
  ...
  const width = columns; // input box and picker span the terminal, like the transcript
  ...
  const inputBlockHeight = pickerOpen
    ? MODELS.length + 3
    : cardOpen
      ? cardRows + 2
  ...
  const lines = items.flatMap((item) =>
    hardWrapAnsi(
      `${renderEntry(item, { verbose, foldHint: "Ctrl+O to expand" })}\n`,
      columns,
    ),
  );
    ...
    <Box flexDirection="column">
      ...
      {pickerOpen ? (
        ...
        <Box
          ...
          width={width}
        >
          {cardLines(card).map((line, i) => (
            // biome-ignore lint/suspicious/noArrayIndexKey: rows re-render wholesale per keystroke
        ...
        >
          {card?.typing === "secret" && (
            <Text wrap="truncate-end">
              {chalk.bold(card.pending.secrets?.[card.step]?.name ?? "secret")}
          ...
          {card?.typing === "custom" && (
            <Text wrap="truncate-end">
              ...
              {chalk.dim(
                "  — your own answer · Enter to save · Esc to go back",
              )}
            </Text>
          )}
          <Box>
      ...
      <Text dimColor wrap="truncate-end">
        {pickerOpen
          ...
          : cardOpen
            ? "  the agent is waiting on you · answer above, or Esc for later"
...
export async function runInteractiveSession(
  ...
  const listeners = new Set<(entry: TranscriptEntry) => void>();
  const buffered: TranscriptEntry[] = [];
  const onLine = (line: TranscriptEntry) => {
    if (listeners.size === 0) {
      buffered.push(line);
      return;
    }
  ...
  const subscribe = (listener: (entry: TranscriptEntry) => void) => {
...
export async function runGenesisSession(
  ...
  const genesis: SessionEngine = {
    ...
    answer(p, action, input) {
      // Nothing can be pending before the app exists; after, the real engine has it.
