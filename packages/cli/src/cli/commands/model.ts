...
async function modelAction(
  ...
  if (!input) {
    ...
    for (const m of MODELS) {
      ...
      const name = active ? theme.styles.bold(m.name) : m.name;
      log.message(`${marker} ${name}${note}`);
    }
    return {
      outroMessage: `Current: ${theme.styles.bold(displayName(current))}. Set with \`base44 model <name>\`.`,
    };
  }
...
export function getModelCommand(): Base44Command {
  const command = new Base44Command("model", {
    requireAppContext: false,
  });
  command
    .description(
      "Pick the builder model used for your turns (account-wide). No argument lists models and shows the current pick; `default` clears it",
    )
