...
export type StreamEvent =
  ...
  | {
      ...
      summary: string;
      ok: boolean;
      /** The tool's full result: the string it returned, or JSON for anything else. */
      result: string;
      /** Parsed arguments; null when the wire truncated the JSON. */
...
/** An announced call plus its parsed arguments, kept for the result line. */
type AnnouncedCall = AnnouncedTool & { args: Record<string, unknown> | null };
...
interface MessageProgress {
  ...
  announcedTools: Map<
    string,
    AnnouncedTool & { args: Record<string, unknown> | null }
  >;
...
/** The whole result as text: strings verbatim, anything else as JSON. */
...
/** Parsed tool arguments, or null when the wire cut the JSON short. */
function toolArgs(
...
const SALIENT_KEYS = ["command", "path", "file_path", "title", "find"];
  ...
  for (const message of messages) {
    ...
    for (const tool of message.tool_calls ?? []) {
      if (!progress.announcedTools.has(tool.id)) {
        ...
        const meta = progress.announcedTools.get(tool.id) as AnnouncedCall;
        events.push({
          kind: "tool_start",
          id: tool.id,
          name: tool.name,
