 ...
 * Honors `.assetsignore` at the assets root with full gitignore semantics
 * (globby reads it through the `ignore` package, so negation works), always
 * skips `.assetsignore`, `wrangler.json`, and `.dev.vars`, rejects files
 * larger than 25 MiB, and caps the total file count at 100,000.
 */
export async function buildAssetManifest(
  ...
  // globby returns forward-slash paths on every platform, which is exactly
  // how the manifest keys them.
  const found = await globby("**/*", {
    cwd: assetsDir,
    dot: true,
    onlyFiles: true,
    followSymbolicLinks: false,
    // `ignoreFiles` hands .assetsignore to the `ignore` package — real
    // gitignore semantics, negation included. Do NOT also pass `ignore` to
    // hide the file itself: globby globs for ignore files using that same
    // option, so it would then find none and silently apply no patterns at
    // all. Drop the always-ignored names from the results instead.
  ...
  for (const relativePath of relativeFilePaths.sort()) {
    ...
    // Stat before read: the size limit exists so a huge file never has to be
    // pulled into memory.
