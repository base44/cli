...
describe("readProjectConfig", () => {
  ...
  it("reads project with entities", async () => {
    const result = await readProjectConfig(resolve(FIXTURES_DIR, "with-entities"));
  ...
  it("reads project with functions and entities", async () => {
    const result = await readProjectConfig(resolve(FIXTURES_DIR, "with-functions-and-entities"));
  ...
  it("reads project with agents", async () => {
    const result = await readProjectConfig(resolve(FIXTURES_DIR, "with-agents"));
  ...
  it("throws when no config file exists", async () => {
    await expect(readProjectConfig(resolve(FIXTURES_DIR, "no-config"))).rejects.toThrow(
      /Project root not found/
    );
  });

  it("throws on invalid JSON syntax", async () => {
    await expect(readProjectConfig(resolve(FIXTURES_DIR, "invalid-json"))).rejects.toThrow();
  });

  it("throws on invalid config schema", async () => {
    await expect(readProjectConfig(resolve(FIXTURES_DIR, "invalid-config-schema"))).rejects.toThrow(
      /Invalid project configuration/
    );
  });

  it("throws on invalid entity file", async () => {
    await expect(readProjectConfig(resolve(FIXTURES_DIR, "invalid-entity"))).rejects.toThrow();
  });

  it("throws on invalid agent file", async () => {
    await expect(readProjectConfig(resolve(FIXTURES_DIR, "invalid-agent"))).rejects.toThrow();
  });

  it("throws on duplicate agent names", async () => {
    await expect(readProjectConfig(resolve(FIXTURES_DIR, "duplicate-agent-names"))).rejects.toThrow(
